...
pf = pf + os.path.sep + 'suite2p_2stepReg_func/plane0/'
save_path = ''
save_path = '/Users/davidh/Data/Freiwald/Analysis/Coconut/20230106d/SP_200umdeep_1p2mm_3umppix_18p82Hz_40mW/s2p_traces'
...
for line in lines:
    ...
    tmp_acqfr = subcol[9].split('=')[1].strip() # CHANGED IN 20230106d!
    tmp_acqfr = int(tmp_acqfr.split('AI_')[0].strip()) # ADDED TO 20230106d!
    tmp_f = subcol[5].split('=')[1].strip()
    tmp_cond = subcol[4].split('=')[1].strip()
    # if tmp_f == 'wn':
    #    tmp_f = 0 
...
#%% Plot histogram for the number of ROIs with corresponding DSI tuning values
...
BF_norm = BF_noblnk_dFFpass / np.max(BF_noblnk_dFFpass)
plot_map(ROIs[dFFpass_ridx], BF_norm, np.ones(BF_norm.shape), tuning_thresh=0, 
...
for r in range(0, n_dFFpass, 1):
    ...
    plt.show()
    if saving:
        fig.savefig(save_path + os.path.sep + 'Coconut_20230106d_s2p_r{}_'.format(r) + \
                    'roi{}_bf{}_'.format(ridx, BF_noblnk_dFFpass[r].astype(int)) + \
                    '.svg',
                    format='svg', dpi=1200)
    plt.pause(0.05)

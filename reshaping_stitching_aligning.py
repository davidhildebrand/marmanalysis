...
if jeff_ch_order:
    ...
    params['chans_order_30planes'] = np.array([ 1,  5,  6,  7,  8,  9,  2, 10, 11, 12, 13, 14, 15, 16, 17,
                                3, 18, 19, 20, 21, 22, 23,  4, 24, 25, 26, 27, 28, 29, 30]) - 1
else:
    params['chans_order_15planes'] = np.linspace(0,14, 15, dtype = int)
    params['chans_order_30planes'] = np.linspace(0,29, 30, dtype = int)
...
if not params['lateral_align_planes']:
    ...
    params['make_nonan_volume'] = False # It is going to be no-nan by definition, no need to check for it
...
del i_dir, params['raw_data_dirs'], params['fname_must_contain'], params['fname_must_NOT_contain']
...
for current_pipeline_step in pipeline_steps:
    ...
    for i_file in list_files_for_reconstruction:
        ...
        if i_file == 0:
            ...
            if params['json_logging']: json_logger.debug(json.dumps({'debug_message':'Number of planes: ' + str(n_planes)}))
        ...
        if current_pipeline_step == 'make_template':
            if params['seams_overlap'] == 'calculate':
                ...
                overlaps_planes_seams_scores = np.zeros((n_planes, n_mrois-1, params['max_seam_overlap']-params['min_seam_overlap']))  # We will avoid i_overlaps = 0
                
                for i_plane in range(n_planes):
                    for i_seam in range(n_mrois-1):
                        for i_overlaps in range(params['min_seam_overlap'],params['max_seam_overlap']):
                            strip_left  = planes_mrois[i_plane, i_seam  ][0, -params['n_ignored_pixels_sides']-i_overlaps:-params['n_ignored_pixels_sides']]
                            strip_right = planes_mrois[i_plane, i_seam+1][0, params['n_ignored_pixels_sides']:i_overlaps +params['n_ignored_pixels_sides']]
                            subtract_left_right = abs(strip_left - strip_right)
                            overlaps_planes_seams_scores[i_plane, i_seam, i_overlaps-params['min_seam_overlap']] = np.mean(subtract_left_right)
                ...
                if params['alignment_plot_checks']:
                    ...
                    for i_plane in range(n_planes):
                        plt.plot(range(params['min_seam_overlap'],params['max_seam_overlap']), overlaps_planes_scores[i_plane])
                    ...
                    for i_plane in range(n_planes):
                        for shift in range(-2, 3):
                            ...
                            for i_mroi in range(n_mrois):
                                ...
                                canvas_alignment_check[x_start:x_end,y_start:y_end,i_mroi%2] = planes_mrois[0,i_plane,i_mroi] - np.min(planes_mrois[0,i_plane,i_mroi])
                ...
                raise Exception('params[\'seams_overlap\'] should be set to \'calculate\', an integer, or a list of length n_planes')
        
        
        #%% Create a volume container
        
        if current_pipeline_step == 'make_template': # For templatingMROIs, we will get here when working on the last file
            n_f = 1
        elif current_pipeline_step == 'reconstruct_all':
        ...
        n_x = len(reconstructed_xy_ranges_si[0]) - min(overlaps_planes) * (n_mrois-1) + max_shift_x
        n_y = len(reconstructed_xy_ranges_si[1]) + max_shift_y
        n_z = n_planes

        if params['json_logging']: json_logger.debug(json.dumps({'debug_message':'Creating volume of shape: ' + str([n_f, n_x, n_y, n_z]) + ' (f,x,y,z)'}))
        ...
        if params['json_logging']: json_logger.debug(json.dumps({'debug_message':'Merging MROIs and placing them into the volume'}))
        for i_plane in range(n_planes):
            ...
            if params['save_output']:
                ...
                if params['save_as_volume_or_planes'] == 'volume':
                    ...
                    for i_plane in range(n_planes):
                        ...
                        if i_file == list_files_for_reconstruction[-1] and params['concatenate_all_h5_to_tif']:
                ...
                if params['save_meanf_png']:
                    if not params['meanf_png_only_first_file'] or i_file == 0:
                        ...
                        for i_plane in range(n_planes):
                            ...
                            pct_low, pct_high = np.nanpercentile(plane_for_png, params['intensity_percentiles'])
                            ...
                            #Place it on canvas
                            x_start = i_plane % columns * (plane_for_png.shape[1]+params['gaps_columns'])
                            y_start = i_plane // columns * (plane_for_png.shape[0]+params['gaps_rows'])
                ...
                if params['save_mp4']:
                    if not params['video_only_first_file'] or i_file == 0:
                        ...
                        if params['video_duration_secs'] != 0:
                            ...
                            use_until_frame_n = -1
                        canvas_video = np.zeros((volume[:use_until_frame_n].shape[0] - params['rolling_average_frames'] + 1, 
                                           (volume.shape[1] + params['gaps_rows']) * rows - params['gaps_rows'],
                                           (volume.shape[2] + params['gaps_columns']) * columns - params['gaps_columns']),
                                           dtype = np.uint8)
                        for i_plane in range(n_planes):
                            plane_for_video = copy.deepcopy(volume[:use_until_frame_n,:,:,i_plane])
                            #Apply rolling average by convolving
                            plane_for_video = scipy.signal.convolve(plane_for_video, np.ones(([params['rolling_average_frames'],1,1])), mode='valid')
                            #Apply percentile-dynamic-range
                            pct_low, pct_high = np.nanpercentile(plane_for_video, params['intensity_percentiles'])
                            ...
                            #Place it on canvas
                            x_start = i_plane % columns * (plane_for_video.shape[2]+params['gaps_columns'])
                            y_start = i_plane // columns * (plane_for_video.shape[1]+params['gaps_rows'])
                        ...
                        input_filename = os.path.basename(path_input_file) 
                        output_filename_video = save_dir + input_filename[:-4] + '_RollingAvg' + str(params['rolling_average_frames']) + 'Frames_Speed' + str(params['video_play_speed']) + 'x.mp4'
